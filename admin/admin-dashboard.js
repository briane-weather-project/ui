...
async function loadOpenMeteoForecast(lat, lng) {
    ...
    const geoPromise = fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&zoom=10`, {
        headers: { 'User-Agent': 'ULAN-Admin-Dashboard/1.0' }
...
async function populateHourlyTempTimeline() {
    ...
    try {
        ...
        // Auto-scroll to "Now" card (it's the first one now)
...
function updateWeatherBackground(rain) {
    let weatherType = "sunny";
    const dangerThreshold = alertThresholdVal;
    const warningThreshold = alertThresholdVal * 0.5;

    if (rain >= dangerThreshold) {
        weatherType = "stormy";
    } else if (rain >= warningThreshold) {
        weatherType = "rainy";
    } else if (rain > 0) {
        weatherType = "cloudy";
    } else {
...
function loadDashboardData() {
    ...
    db.collection('weather').doc('current').onSnapshot(doc => {
        if (doc.exists) {
            ...
            currentRainfallVal = parseFloat(data.rainfall) || 0.0;
            const tempVal = parseFloat(data.temperature) || 0.0;
            const humidityVal = parseFloat(data.humidity) || 0.0;
            const pressureVal = parseFloat(data.pressure) || 0.0;
            ...
            if (tempEl) tempEl.innerText = tempVal !== 0 ? tempVal.toFixed(1) : "--.-";
            if (humidityEl) humidityEl.innerText = humidityVal !== 0 ? humidityVal.toFixed(0) : "--";
            if (pressureEl) pressureEl.innerText = pressureVal !== 0 ? pressureVal.toFixed(1) : "----";
            ...
            updateWeatherBackground(currentRainfallVal);
        } else {
            updateStatusUI("OFFLINE", "#ef4444", false);
    ...
    db.collection('users').onSnapshot(snapshot => {
        ...
        snapshot.forEach(doc => {
            ...
            row.innerHTML = `
                ...
                <td><code style="background:#f1f5f9; padding:2px 6px; border-radius:4px;">${userData.phoneNumber || 'N/A'}</code></td>
                <td>
                ...
                <td>
                    <div style="display: flex; gap: 8px;">
                        ...
                        <button class="text-btn danger" onclick="deleteUser('${doc.id}')">Delete</button>
                    </div>
                </td>
            `;
...
if (pushSimBtn) {
    pushSimBtn.addEventListener('click', async () => {
        ...
        // Rain rate — for simulation, estimate from rainfall value
        let rainRate = 0;
        if (rain > 0) {
            rainRate = rain * 2; // Simple estimate: assume rain fell over ~30min
        }
