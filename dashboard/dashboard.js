...
function startRealtimeUpdates() {
    ...
    let userCurrentFirestoreUnsub = null;
    let usingRtdbCurrent = false;

    const listenFirestoreCurrent = () => {
        if (userCurrentFirestoreUnsub) return;
        userCurrentFirestoreUnsub = db.collection('weather').doc('current').onSnapshot(doc => {
            if (!usingRtdbCurrent) {
                handleCurrentData(doc.exists ? doc.data() : null);
            }
        }, err => {
            console.error("Dashboard Current Snapshot Error:", err);
            handleCurrentData(null);
        });
    };
    ...
    if (rtdb) {
        rtdb.ref('weather/current').on('value', snapshot => {
            ...
            if (val && Object.keys(val).length > 0) {
                ...
                handleCurrentData(val);
            } else {
                usingRtdbCurrent = false;
                listenFirestoreCurrent();
            }
        }, error => {
            console.warn("RTDB current note, using Firestore fallback:", error);
            usingRtdbCurrent = false;
            listenFirestoreCurrent();
        });
    } else {
