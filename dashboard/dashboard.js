    ...
    db.collection('weather').doc('current').onSnapshot(doc => {
        if (!doc.exists) {
            ...
            if (lastSeen) {
                ...
                if (diffMinutes < 5) updateStatusUI("System Active", "#10b981", true);
            ...
            updateStatusAndRisk(rain, water);
...
function updateStatusAndRisk(rain, water) {
    let status = "Safe";
    ...
    // Check water level overflow condition (JSN-SR04T)
    ...
    if (water >= maxChannelCapacity) {
        ...
        weatherType = "rainy";
    } else if (rain >= dangerThreshold) {
        status = "CRITICAL";
        badgeClass = "risk-badge danger";
        risk = `Extreme danger: Flood threshold of ${dangerThreshold.toFixed(2)}mm reached! Seek higher ground.`;
        weatherType = "stormy";
    } else if (rain >= warningThreshold) {
        status = "WARNING";
        badgeClass = "risk-badge warning";
        risk = `Heavy rain: Water levels rising toward threshold of ${dangerThreshold.toFixed(2)}mm.`;
        weatherType = "rainy";
    } else if (rain > 0) {
        status = "ALERT";
        badgeClass = "risk-badge alert-level";
        risk = "Light precipitation detected in the area.";
        weatherType = "cloudy";
    } else {
        const hour = new Date().getHours();
