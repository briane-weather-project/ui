...
// ── Show/Hide Secondary Sensors Toggle (Admin) ──────────────────────────
function initAdminSensorToggle() {
    const toggleBtn = document.getElementById('toggle-admin-more-sensors-btn');
    const drawer = document.getElementById('admin-more-sensors-drawer');
    const toggleIcon = document.getElementById('toggle-admin-sensors-icon');
    const toggleText = document.getElementById('toggle-admin-sensors-btn-text');

    if (!toggleBtn || !drawer || toggleBtn.dataset.initialized) return;
    toggleBtn.dataset.initialized = 'true';

    // Load saved admin preference (defaults to collapsed)
    const isExpanded = localStorage.getItem('briane_admin_sensors_expanded') === 'true';
    if (isExpanded) {
        drawer.classList.remove('collapsed');
        drawer.classList.add('expanded');
        if (toggleText) toggleText.innerText = 'Hide Additional Metrics';
    ...
    toggleBtn.addEventListener('click', () => {
        ...
        if (currentlyExpanded) {
            ...
            if (toggleText) toggleText.innerText = 'Show Additional Metrics (Light, AI Score)';
            if (toggleIcon) toggleIcon.setAttribute('data-lucide', 'chevron-down');
            localStorage.setItem('briane_admin_sensors_expanded', 'false');
        } else {
            drawer.classList.remove('collapsed');
            drawer.classList.add('expanded');
            if (toggleText) toggleText.innerText = 'Hide Additional Metrics';
            if (toggleIcon) toggleIcon.setAttribute('data-lucide', 'chevron-up');
            localStorage.setItem('briane_admin_sensors_expanded', 'true');
        }
        if (window.lucide) lucide.createIcons();
    });
}
document.addEventListener('DOMContentLoaded', initAdminSensorToggle);
setTimeout(initAdminSensorToggle, 300);
...
function loadDashboardData() {
    ...
    // Subscribe to RTDB with Firestore fallback
    if (rtdb) {
        rtdb.ref('weather/current').on('value', snapshot => {
            handleDeviceCurrentData(snapshot.val());
        }, error => {
            console.error("RTDB current error, falling back to Firestore:", error);
    ...
    if (rtdb) {
        rtdb.ref('weather/config').on('value', snapshot => {
            ...
            console.error("RTDB config error, falling back to Firestore:", error);
...
async function saveConfigDual(configData) {
    ...
    if (rtdb) {
        ...
        promises.push(rtdb.ref('weather/config').update(configData));
    }
...
function renderAdminHotlines() {
    ...
    activeHotlines.forEach((h, idx) => {
        ...
        tr.innerHTML = `
            <td>
                <input type="text" class="settings-input hotline-input-org" data-idx="${idx}" value="${escapeHtml(h.org || '')}" style="font-weight: 700; width: 100%;">
            </td>
            <td>
                <input type="text" class="settings-input hotline-input-num" data-idx="${idx}" value="${escapeHtml(h.number || '')}" style="width: 100%;">
            </td>
            <td>
                <input type="tel" class="settings-input hotline-input-call" data-idx="${idx}" value="${escapeHtml(h.callNumber || '')}" style="width: 100%;">
            </td>
            <td>
                <input type="text" class="settings-input hotline-input-sub" data-idx="${idx}" value="${escapeHtml(h.sub || '')}" style="width: 100%;">
            </td>
        `;
...
function renderAdminShelters() {
    ...
    activeShelters.forEach((s, idx) => {
        ...
        card.style.cssText = 'padding: 1.25rem; border-radius: 18px; display: flex; flex-direction: column; gap: 12px;';
        card.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <strong style="font-size: 0.95rem; color: #0f172a;">Shelter #${idx + 1}</strong>
                <button type="button" class="text-btn danger delete-shelter-btn" data-idx="${idx}" style="font-size: 0.75rem; padding: 4px 8px;">
                    <i data-lucide="trash-2" style="width: 14px; height: 14px;"></i> Remove
                </button>
            </div>
            <div class="form-group" style="margin: 0;">
                <label style="font-size: 0.72rem; font-weight: 700;">Shelter Facility Name</label>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
                <div class="form-group" style="margin: 0;">
                    <label style="font-size: 0.72rem; font-weight: 700;">Operational Status</label>
                ...
                <div class="form-group" style="margin: 0;">
                    <label style="font-size: 0.72rem; font-weight: 700;">Max Capacity (Persons)</label>
            ...
            <div class="form-group" style="margin: 0;">
                <label style="font-size: 0.72rem; font-weight: 700;">Location / Address &amp; Elevation</label>
            ...
            <div class="form-group" style="margin: 0;">
                <label style="font-size: 0.72rem; font-weight: 700;">Google Maps Navigation URL</label>
            ...
            <div class="form-group" style="margin: 0;">
                <label style="font-size: 0.72rem; font-weight: 700;">Available Amenities (Comma separated)</label>
...
setTimeout(loadAdminEmergencyDirectory, 500);
