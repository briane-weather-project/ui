...
const saveNotifsBtn = document.getElementById('save-notifs-btn');
if (saveNotifsBtn) {
    saveNotifsBtn.addEventListener('click', async () => {
        const user = auth.currentUser;
        if (!user) return;

        const prefs = {
            flood: document.getElementById('notif-flood')?.checked ?? true,
            system: document.getElementById('notif-system')?.checked ?? true,
            weekly: document.getElementById('notif-weekly')?.checked ?? false,
        };

        try {
            saveNotifsBtn.disabled = true;
            await db.collection('users').doc(user.uid).update({ notificationPrefs: prefs });
            showSettingsMsg('notif-msg', '✓ Notification preferences saved!', true);
        } catch (err) {
            console.error('Notif prefs error:', err);
            showSettingsMsg('notif-msg', 'Failed to save preferences.', false);
        } finally {
            saveNotifsBtn.disabled = false;
        }
    });
}
...
if (signOutAllBtn) {
    signOutAllBtn.addEventListener('click', async () => {
        if (!confirm('This will sign you out immediately. Continue?')) return;
        try {
            await auth.signOut();
            window.location.href = '../index.html';
        } catch (err) {
            showSettingsMsg('danger-msg', 'Sign out failed. Try again.', false);
        }
    });
}
...
if (deleteAccountBtn) {
    deleteAccountBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (err.code === 'auth/wrong-password' || err.code === 'auth/invalid-credential') msg = 'Incorrect password. Deletion cancelled.';
            else if (err.code === 'auth/too-many-requests') msg = 'Too many attempts. Try again later.';
